...
\begin{theorem}
\label{th:hm_paley}
Ak platí, že $n = p^r + 1 \equiv 0 (\textrm{mod}\ 4)$ a zároveň $p$ je nepárne prvočíslo tak existuje Hadamardova matica rádu $n$ a takéto matice voláme Payleyho.
\end{theorem}
\begin{construction}
...
a to podľa predpisu $f_L((r, s)) = (r-1)*6+s$. Ak si teraz zadefinujeme funkciu $f_L^{-1}$ ako inverznú funkciu prevádzajúcu lineárny
...
\begin{equation*}
    h_{i, j} = \begin{cases}
               1               & \text{označme}: \, f_L^{-1}(i) = (a_1, a_2), f_L^{-1}(j) = (b_1, b_2)
               \\              & \text{ak platí}: \, a_1=b_1 \lor a_2=b_2 \lor L_{a_1a_2} = L_{b_1b_2}
               \\-1            & \text{inak}
           \end{cases}
\end{equation*}
\end{center}
