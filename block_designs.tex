...
Nech $X = \set{x_1, \ldots, x_v}$ je množina objektov a $\mathcal{B} = \set{X_1, \ldots, X_v} \subset \powerset(X)$ je množina podmnožín objektov (tieto podmnožiny voláme \emph{bloky}), pričom sú splnené nasledujúce podmienky:
\begin{enumerate}
...
\begin{enumerate}
	...
	$$ A^T J = \frac{k + \lambda (v - 1)}{k} J \quad / transponujeme$$
...
\begin{theorem_hard}{(Typ B dif. množín --- bikvadratické rezíduá)}\\
	...
	$$v = p = 4x^2+1, k = x^2, \lambda = \frac{x^2 - 1}{4}$$
	
	\end{theorem_hard}

\begin{theorem_hard}{(Typ T dif. množín --- Twin prime power dif. množiny)}\\ 
	Nech je množina $D$ v grupe $((GF(q), +) \times (GF(q + 2), +), +)$, kde $q$ aj $q + 2$ sú mocniny prvočísel, definovaná nasledovne $D = \{(x, y) ~|~ y = 0$ alebo x aj y sú nunulové a oba sú súčasne štvorce alebo súčasne neštvorce$\}$. Potom $D$ je $(v, k, \lambda)$-diferenčná množina s parametrami:
	$$v = q^2 + 2q, k = \frac{q^2 + 2q - 1}{2}, \lambda = \frac{q^2 + 2q - 3}{4}$$
\end{theorem_hard}
